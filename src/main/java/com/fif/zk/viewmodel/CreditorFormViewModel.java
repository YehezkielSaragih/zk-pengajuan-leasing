    ...
    public class CreditorFormViewModel {
        ...
        public void save() {
            ...
            if (name == null || name.trim().isEmpty()) {
                Clients.showNotification("Name cannot be empty!", "error", null, "top_right", 3000);
                return;
            }

            // Name minimum 3 characters
            if (name.trim().length() < 3) {
                Clients.showNotification("Name must be at least 3 characters!", "error", null, "top_right", 3000);
                return;
            }
            ...
            if (exists) {
                Clients.showNotification("Creditor with this name already exists!", "error", null, "top_right", 3000);
                return;
            }

            // Age validation
            if (age == null || age < 18) {
                Clients.showNotification("Creditor must be at least 18 years old!", "error", null, "top_right", 3000);
                return;
            }

            // Address validation
            if (address == null || address.trim().isEmpty()) {
                Clients.showNotification("Address cannot be empty!", "error", null, "top_right", 3000);
                return;
            }

            // Income validation
            if (income == null) {
                Clients.showNotification("Income cannot be empty!", "error", null, "top_right", 3000);
                return;
            }
            ...
            // Redirect
            Executions.sendRedirect("layout.zul?page=creditor-dashboard.zul&success=true");
        }
