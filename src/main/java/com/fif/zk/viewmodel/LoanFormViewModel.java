...
public class LoanFormViewModel {
    ...
    public void save() {
        ...
        if (downPayment < minDP) {
            Clients.showNotification("Down payment must be at least 30% of total loan (" + (int)minDP + ")!", "error", null, "middle_center", 4000);
            return;
        }
        if (downPayment >= loanAmount) {
            Clients.showNotification("Down payment must be less than total loan!", "error", null, "middle_center", 4000);
            return;
        }
